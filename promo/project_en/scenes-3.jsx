  ...
  return (
    <Sprite start={58} end={65}>
      {({ localTime, progress }) => {
        ...
        const cards = [
          { kind: 'exhibit',   title: 'Seated Woman',  sub: 'Picasso · 1953',     body: 'Picasso’s mature work. Multiple angles of one face appear at once — taking apart the act of seeing itself.', tag: 'EXHIBIT', x: -260, y: -440, objPos: '50% 50%' },
          { kind: 'person',    title: 'Pablo Picasso', sub: 'Spanish · 1881–1973', body: 'Spanish-born, worked in France. One of the most influential artists of the 20th century.',                tag: 'PERSON',  x:  260, y: -440, objPos: '50% 25%' },
          { kind: 'style',     title: 'Cubism',        sub: '1907 → onward',       body: 'Co-founded by Picasso and Braque. Breaks single-point perspective; multiple views fused into one.',         tag: 'STYLE',   x:  300, y:  500, objPos: '50% 50%' },
          { kind: 'technique', title: 'Multi-viewpoint', sub: 'Method',             body: 'Front and side appear at once. The 3D world flattened to a 2D collage — a new grammar for modern painting.', tag: 'TECHNIQUE', x: -300, y:  500, objPos: '40% 30%' },
          { kind: 'theme',     title: 'Françoise',     sub: 'Françoise Gilot',     body: 'Picasso’s partner and muse around 1953. This painting was finished as their relationship was ending.',     tag: 'THEME',   x:    0, y:  720, objPos: '50% 20%' },
        ];

        const tagColor = { 'EXHIBIT':'#b8924a','PERSON':'#7a6a92','STYLE':'#a4604a','TECHNIQUE':'#5e7a5a','THEME':'#8a7048' };
          ...
          <div style={{ position: 'absolute', inset: 0, background: '#f3eee4', opacity: overall, overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', top: 230, left: 0, right: 0, textAlign: 'center' }}>
              ...
              <div className="serif" style={{ fontSize: 56, color: '#1a1612', fontWeight: 400, marginTop: 14, lineHeight: 1.1 }}>
                One photo,<br/>one web of knowledge
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div className="serif" style={{ fontSize: 18, color: '#1a1612', marginTop: 2 }}>1 photo</div>
              </div>
            </div>
  ...
  return (
    <Sprite start={64.5} end={71}>
      {({ localTime, progress }) => {
        ...
        const periods = [
          { y: '-1400', label: 'Egypt',         src: REAL.mummy },
          { y: '-500',  label: 'Greek',         src: REAL.diana },
          { y: '1100',  label: 'N. Song',       src: REAL.guanyin },
          { y: '1758',  label: 'Rococo',        src: REAL.boucher },
          { y: '1880',  label: 'Impressionism', src: PHOTOS[9] },
          { y: '1953',  label: 'Cubism',        src: REAL.picasso },
        ];
        ...
        return (
          <div style={{ position: 'absolute', inset: 0, background: '#0d0a08', opacity: overall, overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', top: 230, left: 70, color: '#f3eee4' }}>
              ...
              <div className="serif" style={{ fontSize: 56, fontWeight: 400, marginTop: 12, lineHeight: 1.1 }}>A timeline<br/>across 3000 years</div>
            </div>
...
const BUBBLE_DATA_EN = {
  name: 'root',
  children: [
    { name: 'Cubism',        group: 'concept',   value: 100 },
    { name: 'Picasso',       group: 'pioneer',   value: 62  },
    { name: 'Braque',        group: 'pioneer',   value: 55  },
    { name: 'Met',           group: 'museum',    value: 48  },
    { name: 'African Masks', group: 'influence', value: 40  },
    { name: 'Cézanne',       group: 'influence', value: 36  },
    { name: 'Multi-view',    group: 'method',    value: 32  },
    { name: 'Mondrian',      group: 'legacy',    value: 24  },
    { name: 'Abstract Art',  group: 'legacy',    value: 22  },
  ],
};
const BUBBLE_GROUPS_EN = {
...
const BUBBLE_PACK_EN = (() => {
  const w = 900, h = 800;
  const pack = window.d3.pack().size([w, h]).padding(14);
  const root = pack(window.d3.hierarchy(BUBBLE_DATA_EN).sum(d => d.value));
  ...
  return (
    <Sprite start={70.5} end={79}>
      {({ localTime }) => {
        ...
        const { w, h, leaves } = BUBBLE_PACK_EN;
        ...
        return (
          <div style={{ position: 'absolute', inset: 0, background: '#0d0a08', opacity: overall, overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 230, left: 70, color: '#f3eee4' }}>
              <div className="mono" style={{ fontSize: 24, letterSpacing: '0.32em', color: '#d6b56e', textTransform: 'uppercase' }}>VIEW · 02</div>
              <div className="serif" style={{ fontSize: 56, fontWeight: 400, marginTop: 12, lineHeight: 1.1 }}>Knowledge<br/>starts to link</div>
            </div>
            ...
            <svg style={{ position: 'absolute', left: x0, top: y0, width: w, height: h, overflow: 'visible' }}>
              {leaves.map((d, i) => {
                ...
                const g = BUBBLE_GROUPS_EN[d.data.group] || BUBBLE_GROUPS_EN.influence;
                const isConcept = d.data.group === 'concept';
                const showSub = d.r > 48;
                const nameSize = Math.min(d.r * 0.34, 26);
                return (
                  <g key={i} transform={`translate(${d.x},${d.y}) scale(${eased})`} opacity={t}>
                    ...
                    {d.r > 26 && (
                      <text y={showSub ? -8 : 0} textAnchor="middle" dominantBaseline="central"
                        fill={g.text}
                        style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: nameSize, fontWeight: 600 }}>
                        {d.data.name}
                      </text>
                    )}
                    {showSub && (
                      <text y={Math.max(18, d.r * 0.34)} textAnchor="middle" dominantBaseline="central"
                        fill={g.sub}
                        style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: Math.min(d.r * 0.16, 12), letterSpacing: '0.28em' }}>
                        {g.label}
                      </text>
                    )}
                  </g>
            ...
            <div style={{ position: 'absolute', bottom: 200, left: 0, right: 0, textAlign: 'center' }}>
              <div className="mono" style={{ fontSize: 22, color: '#d6b56e', letterSpacing: '0.32em', marginBottom: 24, opacity: clamp((localTime - 3) / 1, 0, 1) }}>BADGES · UNLOCKED</div>
              <div style={{ display: 'flex', justifyContent: 'center', gap: 40 }}>
                {[
                  { icon: '◈', name: 'Bronze Hunter' },
                  { icon: '✺', name: 'Color Detective' },
                  { icon: '◉', name: 'Mini Archaeologist' },
                  { icon: '✦', name: 'Myth Apprentice' },
                ].map((b, i) => {
        ...
        return (
          <div style={{ position: 'absolute', inset: 0, background: '#0a0805', opacity: enter, overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              }}>My<span style={{color:'#d6b56e'}}>·</span>Museum</div>
            </div>
            ...
            <div style={{
              ...
              <div className="serif" style={{
                ...
                fontStyle: 'italic',
              }}>Every pause becomes a page of knowledge.</div>
            </div>
